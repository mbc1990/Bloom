	...
	void Update () {
		...
		if(mission_started) {
			if (in_orbit) {
				...
				transform.parent.RotateAround(new Vector3(0,0,1),ORBIT_SPEED*Time.deltaTime);
				...
				if(Vector3.Distance(transform.parent.position, dest.transform.position) < ORBIT_DIST) {
					...
					print ("cube position: "+transform.parent.position+" desination position: "+dest.transform.position);
